...
2) wrapfs/file.c - Added new file operations structure. Modified ->read, ->write to handle synchronous read and writes for memory mapped files. Defined a new file_operations variable wrapfs_main_mmap_fops to define file operations for mmap option. Added wrapfs_read_update_atime() function as a wrapper to generic_file_aio_read() and to update atime of upper filesystem inode.
...
5) wrapfs/mmap.c - Added new address_operations structure and instantiated a dummy address_operations structure to point it according to the mount options. I have copied code from ecryptfs and old unionfs sources. I have used ->readpage, ->write_begin, ->bmap and->write_end from ecryptfs and ->writepage from unionfs (old source code before unionfs was switched to vm_fault operations).

6) In wrapfs/mmap.c I have also added encrypt() and decrypt() functions which shift the english alphabets by a position of 3 places.

7) wrapfs/super.c - Added wrapfs_parse_options to parse mount options. Necessary structures needed for parsing are also added in this file.

8) wrapfs/Makefile - I have edited the Makefile to take one more flag to include conditional compiling. I have also added commands to compile and clean the user programs accordingly.
